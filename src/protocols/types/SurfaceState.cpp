...
void SSurfaceState::updateFrom(SSurfaceState& ref, bool accumulateDamage) {
    CRegion previousBufferDamage;
    if (accumulateDamage)
        previousBufferDamage = accumulateBufferDamage();
    ...
    const bool BUFFER_SPACE_CHANGED = ref.updated.bits.buffer || ref.updated.bits.transform || ref.updated.bits.scale || ref.updated.bits.viewport;
    if (BUFFER_SPACE_CHANGED)
        bufferDamage.add(CBox{{}, bufferSize});
    else
        bufferDamage.add(previousBufferDamage);
}
